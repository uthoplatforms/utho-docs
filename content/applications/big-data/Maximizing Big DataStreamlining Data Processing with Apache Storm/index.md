                                                    ...
                                                    - `DATACENTER_FOR_CLUSTER`
                                                      ...
                                                      - `CLUSTER_SIZE`
                                                        ...
                                                        - `CLUSTER_MANAGER_USES_PUBLIC_IP`
                                                          ...
                                                          Set this value to `false` if the cluster manager node is located in the *same* Utho data center as the cluster nodes. This is the recommended value. Change to `true` **only** if the cluster manager node is located in a *different* Utho data center from the cluster nodes.
                                                                           ...
                                                                           3. Open the newly created `storm-cluster1.conf` file and make changes as described below. Properties that must be entered or changed from their default values are marked as **REQUIRED**:
                                                                              ...
                                                                              - `DATACENTER_FOR_CLUSTER`
                                                                                ...
                                                                                - `NIMBUS_NODE`
                                                                                  ...
                                                                                  This specifies the Utho plan to use for the Nimbus node, which is responsible for distributing and coordinating a Storm topology to supervisor nodes.
                                                                                                        ...
                                                                                                        4. Run the following commands to start the preinstalled word count example topology:
                                                                                                           ...
                                                                                                           3. Transfer the topology JAR from your local workstation to client node:
                                                                                                              ...
                                                                                                              4. Log in to the client node as `clusteradmin`, substituting the appropriate values:
                                                                                                                 ...
                                                                                                                 ./storm-cluster-Utho.sh add-nodes storm-cluster1 api_env_Utho.conf "4GB:3"
                                                                                                                 Or, to add a 2GB and two 4GB supervisor nodes to `storm-cluster1`:

                                                                                                                 ./storm-cluster-Utho.sh add-nodes storm-cluster1 api_env_Utho.conf "2GB:1 4GB:2"
                                                                                                                 ...
                                                                                                                 ./storm-cluster-Utho.sh stop storm-cluster1 api_env_Utho.conf
                                                                                                                 ...
                                                                                                                 ./storm-cluster-Utho.sh destroy storm-cluster1 api_env_Utho.conf
                                                                                                                 ...
                                                                                                                 ./storm-cluster-Utho.sh run storm-cluster1 "apt-get update"
                                                                                                                 ...
                                                                                                                 1. If the files are not already on your cluster manager node, you will first need to copy them from your workstation. Substitute `local-file` for the name or path of the file on your local machine, and `PUBLIC-IP-OF-CLUSTER-MANAGER-Utho` for the IP address of the cluster manager node. You can also specify a different filepath and substitute it for `~`:

                                                                                                                    scp -i ~/.ssh/clustermgr local-files clustermgr@PUBLIC-IP-OF-CLUSTER-MANAGER-Utho:~

                                                                                                                    2. Log in to the Cluster Manager Utho as `clustermgr` and navigate to the `storm-Utho` directory:
                                                                                                                       ...
                                                                                                                       3. Execute the `cp` command, specifying the destination directory on each node and the list of local files to copy:
                                                                                                                          ...
                                                                                                                          ./storm-cluster-Utho.sh cp target-cluster-name "target-directory" "local-files"
                                                                                                                          ...
                                                                                                                          ./storm-cluster-Utho.sh cp storm-cluster1 "~" "~/*.data"

                                                                                                                          ### Delete a Storm Image

                                                                                                                          To delete a Storm image, use the `delete-image` command:

                                                                                                                          ./storm-cluster-Utho.sh delete-image storm-image1 api_env_Utho.conf
                                                                                                                          ...
                                                                                                                          ./zookeepercluster-Utho.sh describe zk-cluster1
                                                                                                                          ...
                                                                                                                          Stopping a Zookeeper cluster cleanly stops the Zookeeper daemon on all nodes, and shuts down all nodes. The cluster can be restarted later. Note that the nodes **will** still incur Utho's hourly charges when stopped.
                                                                                                                          ...
                                                                                                                          ./zookeeper-cluster-Utho.sh stop zk-cluster1 api_env_Utho.conf

                                                                                                                          ### Destroy a Zookeeper Cluster

                                                                                                                          Destroying a Zookeeper cluster permanently deletes all nodes of that cluster and their data. Unlike a Utho that is only shut down, destroyed or deleted Uthos no longer incur hourly charges.
                                                                                                                          ...
                                                                                                                          ./zookeeper-cluster-Utho.sh destroy zk-cluster1 api_env_Utho.conf
                                                                                                                          ...
                                                                                                                          ./zookeeper-cluster-Utho.sh run zk-cluster1 "apt-get update"
                                                                                                                          ...
                                                                                                                          1. If the files are not already on your cluster manager node, you will first need to copy them from your workstation. Substitute `local-file` for the name or path of the file on your local machine, and `cluster-manager-IP` for the IP address of the cluster manager node. You can also specify a different filepath and substitute it for `~`:
                                                                                                                             ...
                                                                                                                             2. Log in to the Cluster Manager Utho as `clustermgr` and navigate to the `storm-Utho` directory:
                                                                                                                                ...
                                                                                                                                3. Execute the `cp` command, specifying the destination directory on each node and the list of local files to copy:
                                                                                                                                   ...
                                                                                                                                   ./zookeeper-cluster-Utho.sh cp target-cluster-name "target-directory" "local-files"
                                                                                                                                   Remember to specify the target directory _before_ the list of source files (this is the reverse of regular `cp` or `scp` commands).
