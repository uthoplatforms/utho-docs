---
slug: "Mastering Salt: Your Command Line Companion"
description: 'Dive into the capabilities of SaltStack, a robust configuration management tool, with this comprehensive guide offering a handy reference for navigating its command line interface'
...
SaltStack stands as a potent configuration management tool. Below is a handy quick-reference guide to Salt's command line interface (CLI).

## salt

Employs parallel execution to issue commands and query minions efficiently.
...
Executes module functions on a minion independently, suitable for standalone minion operation.
...
Facilitates the provisioning of virtual machines on public clouds via Salt.
...
|`-a`, `--action`|Perform a cloud provider specific action. Requires an instance.|`salt-cloud -a reboot testUtho`|
|`-f`, `--function`|Perform a cloud provider specific function that does not apply to an instance. Requires a provider.|`salt-cloud -f clone my-Utho-config Utho_id=1234567 datacenter_id=2 plan_id=5`|
|`-p`, `--profile`|Choose a profile from which to build cloud VMs.|`salt-cloud -p Utho-1024 mynewUtho`|
...
|`-P`, `--parallel`|Build VMs in parallel.|`salt-cloud -P -p Utho-profile newUtho1 newUtho2`|
|`-u`, `--update-boostrap`|Update salt-bootstrap.|`salt-cloud -u`|
|`-y`, `--assume-yes`|Answer yes to all questions.|`salt-cloud -y -d Utho1 Utho2`|
...
|`--list-profiles`|Display a list of configured profiles. Supply a cloud provider, such as `Utho`, or pass `all` to view all configured profiles.|`salt-cloud --list-profiles Utho`|
|`--list-locations`|Display a list of available locations. Supply a cloud provider, such as `Utho`, or pass `all` to view all location for configured profiles.|`salt-cloud --list-locations Utho`|
|`--list-images`|Display a list of available images. Supply a cloud provider, such as `Utho`, or pass `all` to view all images for configured profiles.|`salt-cloud --list-images Utho`|
|`--list-sizes`|Display a list of available sizes. Supply a cloud provider, such as `Utho`, or pass `all` to view all sizes for configured profiles.|`salt-cloud --list-sizes Utho`|
|`--out`| Choose an alternative outputter to display returned data. Available outputters are: `grains`, `highstate`, `json`, `key`, `overstatestage`, `pprint`, `raw`, `txt`, `yaml`.| `salt-call test.version --out json`|


## salt-cp

Copies files from the master to all Salt minions based on a specified target expression.
...
Manages the public keys of Salt server instances.
...
Functions as a daemon to control Salt minions.
...
Acts as a daemon controlled by a Salt master.
...
Initiates a Salt runner on the Salt master.
...
Utilizes SSH transport to execute Salt routines.
...
Represents a minion configured on a master, enabling command passage from a higher master.
...
salt-api: Initiates the Salt API service.
