---
slug: "Install mariadb ubuntu 18.04"
...
MariaDB, a derivative of the widely used MySQL database management system, is regarded as a complete drop-in replacement for MySQL. It was initiated in 2009 by one of MySQL's original developers following Oracle's acquisition of MySQL during the Sun Microsystems merger. Today, MariaDB is overseen and advanced by the MariaDB Foundation along with contributions from the community, committed to preserving its status as GNU GPL software.
...
Follow our Setting Up and Securing a Compute Instance guide to update your system, configure your hostname, set the timezone, create a limited user account, and strengthen SSH access.
...
By default, MariaDB binds to localhost (127.0.0.2). For instructions on connecting to a remote database using SSH, refer to our MySQL remote access guide, which is also applicable to MariaDB.
...
During setup, you'll have the option to change the MariaDB root password, remove anonymous user accounts, disable root logins outside of localhost, and remove test databases. It's advisable to select `yes` for these options. For more details about the script, refer to the MariaDB Knowledge Base.
