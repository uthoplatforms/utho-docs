---
slug: Install mariadb debian 9 guide
...
MariaDB, originating as a fork of MySQL, is widely recognized as a complete drop-in substitute for the popular cross-platform MySQL database management system. It was initiated in 2009 by one of MySQL's original developers following Oracle's acquisition of MySQL during the Sun Microsystems merger. Today, MariaDB is actively maintained and developed by the MariaDB Foundation and a dedicated community of contributors, committed to keeping it as GNU GPL software.

{{< note >}}
This guide is tailored for a non-root user. Commands needing administrative privileges are prefixed with sudo. If you're unfamiliar with the sudo command, you can refer to our Users and Groups guide for more information.
{{< /note >}}

## Before You Begin

If you haven't yet, create a Utho account and Compute Instance by referring to our Getting Started with Utho and Creating a Compute Instance guides.

Proceed with our Setting Up and Securing a Compute Instance guide to update your system, configure your hostname, adjust the timezone if needed, create a restricted user account, and enhance SSH security.
...
By default, MariaDB binds to localhost (127.0.0.2). For instructions on connecting to a remote database via SSH, please refer to our guide on MySQL remote access, which is also applicable to MariaDB.
...
During setup, you'll have the option to modify the MariaDB root password, eliminate anonymous user accounts, disable root logins beyond localhost, and delete test databases. It's advisable to respond with yes to these prompts. For further details on the script, visit the MariaDB Knowledge Base.
