...
{{< note >}}
The steps outlined in this guide require root privileges. Ensure that you execute the steps below as root or with the sudo prefix. For more information on managing privileges, refer to our Users and Groups guide.
{{< /note >}}
...
2.  Create a sample database and populate it with sample data. Execute the following commands to create a database named testdb, owned by a new user `testuser`. These commands also set the password secretpassword for the new user.
...
For more information about SQL commands, visit the SQL Commands page on the MariaDB Knowledge Base. To view MariaDB's command list from within the client, type:
...
"To configure MariaDB according to your preferences, edit the `/etc/mysql/my.cnf` file. This file manages most of the server system variables, typically left at their default settings."
...
{{< note type="alert" >}}
Exposing a MariaDB server to the internet reduces its security. If you need remote access, ensure you set up firewall rules to allow connections only from specific IP addresses.
{{< /note >}}
...
3. Configure MariaDB to listen on all network interfaces. Open the `/etc/mysql/my.cnf` file and modify the `bind-address` variable to include all network interfaces:"
