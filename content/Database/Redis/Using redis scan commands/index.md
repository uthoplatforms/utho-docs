...
Review our Getting Started with Utho guide and follow the instructions to set your Utho's hostname and timezone.

Throughout this guide, utilize sudo whenever necessary. Refer to our How to Secure Your Server guide to establish a standard user account, enhance SSH access security, and disable unnecessary network services.
...
Follow the steps outlined in our How to Install a Redis Server guide to install a Redis server and command-line interface (CLI). Ensure to select your Linux distribution from the drop-down menu at the top of the page for the specific installation steps.

{{< note >}}
This guide is tailored for non-root users. Commands that need elevated privileges are prefixed with sudo. If you're unfamiliar with the sudo command, refer to the Linux Users and Groups guide for more information.
{{< /note >}}
...
Now you're prepared to utilize SCAN commands effectively on your Redis instance, offering efficient solutions for managing large and intricate datasets. The tools covered in this guide empower you to optimize these commands for effective data handling.

Interested in further Redis insights to maximize your server's efficiency? Explore our comprehensive Redis guides covering data types, configurations, and more.