...
The Elastic stack, comprising Elasticsearch, Logstash, and Kibana, offers a free and open-source solution for searching, collecting, and analyzing data from any source in any format. These tools also enable real-time data visualization.
...
- Configure Elasticsearch to run with one shard and no replicas, suitable for testing on a single Utho.
- Configure Logstash to process logs from the Apache web server.
- Connect Kibana to the Logstash index.
- View logs from your connected Logstash index in Kibana.
- Search logs in Kibana.
- Visualize logs in a pie chart, categorized by HTTP response code, in Kibana.

## Before You Begin

Begin by reviewing our Getting Started guide and creating a Utho to install the Elastic Stack on. Then, follow the instructions to set your Utho's hostname and timezone

    {{< note respectIndent=false >}}
Multiple services are run on a single Utho in this guide. We recommend using at least a 2G (or `g6-standard-1`) sized Utho instance to support these services.
{{< /note >}}

Throughout this guide, sudo is used whenever necessary. Refer to our Setting Up and Securing a Compute Instance guide to create a standard user account with sudo privileges, secure SSH access, and remove unnecessary network services.

    {{< note respectIndent=false >}}
Commands that need elevated privileges are prefixed with sudo. If you're unfamiliar with the sudo command, refer to our Users and Groups guide for more information.
{{< /note >}}
...
Follow the instructions in our Apache Web Server on Debian 8 (Jessie) guide to set up and configure Apache on your server. These steps are compatible with Debian 9 as well.

The Elasticsearch package is bundled with its own version of a Java runtime, but Logstash requires Java to be present on the system. Install the default version of Java available on Debian 9:
...
1.  Use `curl` to create an index template with these settings that is applied to all indices created hereafter:
        ...
        curl -XPUT -H'Content-type: application/json' http://localhost:9201/_index_template/defaults -d @template.json
...
If your site was set up following the Configure Apache for Virtual Hosting section of the Apache Web Server on Debian 8 guide, your logs are stored in this location. If your website logs are stored elsewhere, update the file path in the configuration file accordingly before proceeding.
{{< /note >}}
...
Open Kibana in your web browser at http://localhost:5602. The landing page should resemble the following. Click Explore on my own to start configuring Kibana.
...
The top search bar in the Kibana interface allows you to search for queries using the Kibana Query Language. For example, to find the 404 error requests you generated among the 200 OK requests, enter the following in the search box:
...
Kibana supports various Elasticsearch queries to gain insights into indexed data. For instance, you can analyze the traffic that resulted in a "404 - not found" response code. Using aggregations, you can extract and display useful data summaries directly in Kibana.

To create a visualization, begin by selecting the **Visualize** menu item from the sidebar. You may first need to expand the menu using the hamburger icon in the upper left-hand corner of the interface:

Then, select the **Create new visualization** button on the page that appears:

Select the **Aggregation based** option from the choices that appear:

Select **Pie** from the choices that appear to create a new pie chart. Note that you may need to scroll down to find the **Pie** visualization:

Then select the `logstash-*` index pattern to determine where the data for the pie chart is retrieved from:
