...
The Elastic stack, consisting of Elasticsearch, Logstash, and Kibana, offers a free and open-source solution for searching, collecting, and analyzing data from any source and in any format. Additionally, these tools enable real-time data visualization.
...
Configure Elasticsearch to use one shard and no replicas, suitable for testing on a single Utho.

Configure Logstash to process logs from the Apache web server.

Connect Kibana to the Logstash index.

View the logs from the connected Logstash index in Kibana.

Search the logs in Kibana.

Visualize the logs in a pie chart categorized by HTTP response code in Kibana.

## Before You Begin

Follow our Getting Started guide to create a Utho for installing the Elastic stack. Then, set your Utho's hostname and timezone as instructed.
...
This guide uses sudo for commands requiring elevated privileges. Follow the steps in our Setting Up and Securing a Compute Instance guide to create a standard user account with sudo privileges, secure SSH access, and disable unnecessary network services.
...
Follow the steps in our Apache Web Server on Debian 10 (Buster) guide to set up and configure Apache on your server.

The Elasticsearch package is bundled with its own version of a Java runtime, but Logstash requires Java to be present on the system. Install the default version of Java available on Debian 10:
...
If you've set up your site following the Configure Apache for Virtual Hosting section of the Apache Web Server on Debian 10 guide, your logs will be stored in this location. If your website logs are stored elsewhere, update the file path in the configuration file before proceeding.
{{< /note >}}
...
Next, open Kibana in your browser at http://localhost:5602. The landing page should look similar to the following. Click on Explore on my own to begin configuring Kibana.
...
The search bar at the top of the Kibana interface enables you to use the Kibana Query Language to search for results. For instance, if you want to find 404 error requests among 200 OK requests, enter the following query in the search box:
...
Kibana offers various Elasticsearch query capabilities to analyze indexed data effectively. For instance, you can investigate traffic resulting in a "404 - not found" response code. By utilizing aggregations, Kibana can extract and display informative data summaries directly.
